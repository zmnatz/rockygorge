...
export const handler = async (event: any) => {
    if (event.httpMethod === 'POST') {
        try {
            ...
            const FILE_PATH = 'src/data/links.yml';
            ...
            const branchName = `admin-links-${Date.now()}`;
            ...
            await octokit.rest.pulls.create({
                owner: OWNER,
                repo: REPO,
                title: 'Update links.yml',
                head: branchName,
                base: baseBranch,
                body: 'Updated links data via admin page',
            });

            return {
                statusCode: 200,
                body: JSON.stringify({ message: 'Successfully updated links data and created a PR' }),
            };
        } catch (error: any) {
            console.error(error);
            return {
