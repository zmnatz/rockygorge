...
export const handler = async (event: any) => {
    if (event.httpMethod === 'POST') {
        try {
            ...
            const FILE_PATH = 'src/data/calendar.yml';
            ...
            const branchName = `admin-calendar-${Date.now()}`;
            ...
            await octokit.rest.pulls.create({
                owner: OWNER,
                repo: REPO,
                title: 'Update calendar.yml',
                head: branchName,
                base: baseBranch,
                body: 'Updated calendar data via admin page',
            });

            return {
                statusCode: 200,
                body: JSON.stringify({ message: 'Successfully updated calendar data and created a PR' }),
            };
        } catch (error: any) {
            console.error(error);
            return {
