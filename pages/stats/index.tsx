...
export async function getStaticProps() {
  ...
  const playerColumns = [
    ...
    ...Object.keys(firstPlayer)
      .filter((key) => key !== "name" && key !== "game")
      .map((key) => ({
  ...
  return {
    props: { playerColumns, playerDataByGame, gameList, teamColumns, teamData },
  };
}
...
export default function StatsPage({
  ...
  const displayPlayerColumns = useMemo(() => {
    ...
    return cols.filter((col) => {
      if (col.key === "game" || col.key === "name" || col.key === "season")
        return true;
      return Object.entries(STAT_CATEGORIES).some(
        ([category, fields]) =>
        ...
        <Box>
          ...
          >
            <FormControl sx={{ minWidth: 200 }}>
              ...
              >
                <MenuItem value="">
                  <em>All Games (Aggregated)</em>
                </MenuItem>
                <MenuItem value="all_detailed">
                  <strong>Show All Games (Detailed)</strong>
                </MenuItem>
                <MenuItem disabled>
                  <em>Specific Game:</em>
                </MenuItem>
          ...
          <SortableTable
            ...
            renderCell={(col, row) => {
              if (col.key === "name") {
                return (
                  <Link
                    href={`/stats/${slugify(row[col.dataIndex])}`}
                    passHref
                    legacyBehavior
                  >
              ...
              if (col.key === "game") {
                return (
                  <Link
                    href={`/stats/game/${slugify(row[col.dataIndex])}`}
                    passHref
                    legacyBehavior
                  >
                    <MuiLink underline="hover">{row[col.dataIndex]}</MuiLink>
