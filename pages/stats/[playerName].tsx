...
import { formatColumnTitle, STAT_CATEGORIES } from "@/utils/stats";
    ...
    new Set(
      games.flatMap((game) =>
        Array.isArray(game.players)
          ? (game.players as any[]).map((player) => player.name as string)
          : [],
      ),
    ),
  );
...
export async function getStaticProps({ params }) {
  ...
  const allColumns = [
    ...
    ...Object.keys(firstPlayerStats)
      .filter(
        (key) =>
          key !== "name" && key !== "game" && key !== "key" && key !== "season",
      )
      .map((key) => ({
