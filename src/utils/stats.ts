...
export const TEAM_STAT_CATEGORIES: Record<string, string[]> = {
  ...
  Penalties: [
    "total_penalties_gorge",
    "total_penalties_opponent",
    "total_knocks_gorge",
    "total_knocks_opponent",
    "lineouts_lost",
    "scrums_lost",
  ],
};
    ...
    Object.keys(row).forEach((key) => {
      if (key !== "name" && key !== "game" && key !== "key") {
        ...
        if (typeof value === "number") {
          aggregated[playerName][key] =
            (aggregated[playerName][key] || 0) + value;
        } else if (typeof value === "string" && !isNaN(parseFloat(value))) {
          aggregated[playerName][key] =
            (aggregated[playerName][key] || 0) + parseFloat(value);
        } else {
