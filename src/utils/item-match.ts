 ...
 *  Subscription Plans — each plan's `name`, its hosted-button `id`, and the
 *  `value` of each selectable option (the chosen tier). The generic plan
 ...
 *  id, or chosen option value). The item's keys are prepared once, so the
 *  predicate is safe to reuse across many transactions. */
