...
export function CalendarCard({ calendars }: CalendarCardProps) {
  ...
  return (
    <Card>
      <CardHeader
        title={calendars.length > 1 ? "Calendar" : calendars[0]}
        component={Link}
        href="/calendar"
      />
      <CardContent component={List}>
        ...
        {error && (
          <MuiLink component={Link} href="/calendar">
...
function CalendarSection({
  ...
  return (
    <>
      ...
      {data.length < 1 && (
        <ListItem>
          <ListItemText secondary={`No upcoming ${title.toLowerCase()}`} />
        </ListItem>
      )}
      {data.map((item) => (
